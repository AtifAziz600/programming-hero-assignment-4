...
export const getAllGear = async (filters: {
  category?: string; minPrice?: string; maxPrice?: string; brand?: string;
}) => {
...
export const createGear = async (providerId: string, data: any) => {
  return prisma.gearItem.create({ data: { ...data, providerId } });
};

export const updateGear = async (providerId: string, gearId: string, data: any) => {
  const gear = await prisma.gearItem.findUnique({ where: { id: gearId } });
  if (!gear) throw new Error("Gear not found");
  if (gear.providerId !== providerId) throw new Error("Not your gear item");
