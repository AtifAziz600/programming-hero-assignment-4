...
export const createRentalOrder = async (
  customerId: string,
  data: { startDate: string; endDate: string; items: { gearItemId: string; quantity: number }[] }
) => {
  ...
  return prisma.$transaction(async (tx) => {
    ...
    for (const item of data.items) {
      ...
      if (!gear) throw new Error(`Gear item ${item.gearItemId} not found`);
      if (gear.stock < item.quantity) throw new Error(`Not enough stock for ${gear.name}`);
...
export const getUserRentals = async (customerId: string) => {
...
export const getRentalById = async (id: string, userId: string, role: string) => {
  ...
  if (role === "CUSTOMER" && rental.customerId !== userId) throw new Error("Forbidden");
